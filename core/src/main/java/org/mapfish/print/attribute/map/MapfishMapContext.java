...
public final class MapfishMapContext {
  ...
  /**
   * Return the map bounds rotated with the set rotation. The bounds are adapted to rounding changes
   * of the size of the paint area.
   *
   ...
   */
  public MapBounds getRotatedBounds(
      final Rectangle2D.Double paintAreaPrecise, final Rectangle paintArea) {
  ...
  /**
   * Return the map bounds rotated with the set rotation. The bounds are adapted to rounding changes
   * of the size of the set paint area.
   *
   * @return Rotated bounds.
   */
  public MapBounds getRotatedBoundsAdjustedForPreciseRotatedMapSize() {
    ...
    Rectangle paintArea =
        new Rectangle(MapfishMapContext.rectangleDoubleToDimension(paintAreaPrecise));
  ...
  public double getRoundedScaleDenominator(final boolean geodetic) {
    final double scaleDenominator =
        this.bounds
            .getScale(getPaintArea(), this.dpi)
            .getDenominator(
                geodetic, getBounds().getProjection(), this.dpi, getBounds().getCenter());
  ...
  private double getRotatedMapWidth() {
    ...
    if (!FloatingPointUtil.equals(this.rotation, 0.0)) {
      ...
      width =
          Math.abs(width * Math.cos(this.rotation)) + Math.abs(height * Math.sin(this.rotation));
    }
    return width;
  }
  ...
  private double getRotatedMapHeight() {
    ...
    if (!FloatingPointUtil.equals(this.rotation, 0.0)) {
      double width = this.mapSize.getWidth();
      height =
          Math.abs(height * Math.cos(this.rotation)) + Math.abs(width * Math.sin(this.rotation));
    }
    return height;
  }
   ...
   * derived from. Normally there are some parameters that have been changed for this context from
   * the parent. An example of when there might be a parent is when the child has been rotated and
   * has a bounds to envelope the original bounds. It can be useful in some cases to be able to
   * access the parent (and original bounds).
   *
