...
public class Configuration implements ConfigurationObject {
  ...
  static {
    ...
    map.put(
        Geometry.class.getSimpleName().toLowerCase(), Geometry.class.getSimpleName().toLowerCase());
    ...
    map.put(
        Polygon.class.getSimpleName().toLowerCase(), Polygon.class.getSimpleName().toLowerCase());
  ...
  @Autowired private StyleParser styleParser;
  @Autowired private ClientHttpRequestFactory clientHttpRequestFactory;
  ...
  /**
   * If true then all vector layers (and other parts of the system that can be either SVG or Bitmap,
  ...
  /**
   * Get the http credentials. Should also getProxies since {@link org.mapfish.print.http.HttpProxy}
   * is a subclass of {@link org.mapfish.print.http.HttpCredential}.
   */
   ...
   * configuration object also has options for declaring the credentials.
   *
  ...
  /**
   * Configuration for proxying http requests. Each proxy can be configured with authentication and
   * with the uris that they apply to.
   *
   * <p>See {@link org.mapfish.print.http.HttpProxy} for details on how to configure them.
   *
  ...
  public final void printClientConfig(final JSONWriter json) throws JSONException {
    ...
    accessibleTemplates.entrySet().stream()
   ...
   * org.mapfish.print.config.Template#setOutputFilename(String)} and the outputFilename parameter
   * in the request JSON.
   *
   ...
   * format rules are the rules in <a
   ...
   *
   * <p>Example: <code>outputFilename: print-${dd-MM-yyyy}</code> should output: <code>
   * print-22-11-2014.pdf</code>
   *
   * <p>Note: the suffix will be appended to the end of the name.
   *
  ...
  private Symbolizer createMapOverviewStyle(
      @Nonnull final String normalizedGeomName, @Nonnull final StyleBuilder builder) {
  ...
  /**
   * If true and the request JSON has extra parameters in the layers definition, exceptions will be
   * thrown. Otherwise the information will be logged.
   */
  ...
  public final List<Throwable> validate() {
    ...
    if (this.configurationFile == null) {
      validationErrors.add(
          new ConfigurationException(
              "Configuration file is field on configuration " + "object is null"));
    }
    ...
    try {
      ...
      validationErrors.add(
          new ConfigurationException("Cannot parse transparentTileErrorColor", ex));
    }
   ...
   *
   * <p>Any user without the required role will get an error when trying to access any of the
   * templates and no templates will be listed in the capabilities requests.
   *
   ...
   * </code></pre>
   *
   * <p>By default, the referrer is not checked
   *
