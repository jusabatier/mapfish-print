...
public class Template implements ConfigurationObject, HasConfiguration {
  ...
  @Autowired private ClientHttpRequestFactory httpRequestFactory;
  ...
  /**
   * Get the merged configuration between this template and the configuration's template. The
   * settings in the template take priority over the configurations settings but if not set in the
   * template then the default will be the configuration's options.
   */
  ...
  public final java.util.Optional<Style> getStyle(final String styleName) {
    ...
    if (styleRef != null) {
      style =
          (Optional<Style>)
              this.styleParser.loadStyle(getConfiguration(), this.httpRequestFactory, styleRef);
    } else {
  ...
  /**
   * The roles required to access this template. If empty or not set then it is a <em>public</em>
   ...
   *
   * <p>Any user without the required role will get an error when trying to access the template and
   * the template will not be visible in the capabilities requests.
   *
  ...
  /**
   * If set to true (defaults to false), the generated maps, scalebar and north arrow will not
   * contain any transparent images.
   *
   * <p>This is needed in case you want to output PDF/A-1a reports.
   *
   * <p>In pdfA mode, all layers are merged into a single JPEG layer at the requested resolution:
   * WMTS tiles will be downscaled and vector layers will be rendered as bitmaps.
   *
