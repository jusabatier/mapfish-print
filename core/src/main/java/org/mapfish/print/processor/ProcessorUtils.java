...
public final class ProcessorUtils {
  ...
  /**
   * Create the input object required by the processor and populate all the fields from the values
   * object.
   *
   * <p>If {@link Processor#createInputParameter()} returns an instance of values then the values
  ...
  public static <IN, OUT> IN populateInputParameter(
      final Processor<IN, OUT> processor, @Nonnull final Values values) {
    ...
    if (inputObject != null) {
      ...
      for (Field field : fields) {
        String name =
            getInputValueName(
                processor.getOutputPrefix(), processor.getInputMapperBiMap(), field.getName());
  ...
  public static void writeProcessorOutputToValues(
      final Object output, final Processor<?, ?> processor, final Values values) {
